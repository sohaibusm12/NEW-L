...
module.exports = (client) => {
    readdirSync("./commands/").forEach(dir => {
        ...
        for (let file of commands) {
            ...
            if (pull.name) {
                ...
                table.addRow(file, '<a:t_:932037012356218911>')
            } else {
                table.addRow(file, '<a:NO:931955213303160913> -> Missing a help.name, or help.name is not a string.')
                continue;
    ...
    client.on("ready", () => {
        ...
        client.user.setActivity(`${config.prefix}ᴀǫᴜᴀ ᴍᴜsɪᴄ 𝟸𝟺/𝟽`, {
            type: 'Playing'
        });
    });
    client.distube
        ...
        .on("searchCancel", (message) => {
            try {
                ...
                message.react("<a:NO:931955213303160913>")
            } catch (error) {
                console.error(error)
                functions.embedbuilder(client, 5000, message, config.colors.no, "ERROR: ", "```" + error.toString().substr(0, 100) + "```" + "\n\n**Error got sent to my owner!**")
                functions.errorbuilder(error.stack.toString().substr(0, 2000))
